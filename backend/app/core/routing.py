...
class RoutingConfig:
    ...
    type: RoutingType
    target: str | None
    cross_agent: str | None = None

    @classmethod
    def from_dict(cls, data: dict | None) -> "RoutingConfig":
        """Create RoutingConfig from dictionary (from database or config)."""
        if not data:
            return cls(type=RoutingType.SERVICE, target=None)
        return cls(type=RoutingType(data["type"]), target=data.get("target"), cross_agent=data.get("cross_agent"))
